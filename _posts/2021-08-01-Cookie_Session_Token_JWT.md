...
Cookie，有时也用其复数形式 Cookies，指的是由服务端生成, 保存在客户端的一种数据存储形式, 内部以 key/value 键值对形式存储. 一般不存储敏感信息. 内容有时会被加密.Cookie 以键值对 Key-Value 形势进行信息的存储，Cookie 基于域名安全，不同域名的 Cookie 是不能互相访问的
